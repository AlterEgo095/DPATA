...
export function tokenize(text: string): string[] {
  ...
  return { vocabulary: vocab, idf };
    ...
    P_body("Le corpus pilote a été constitué pour valider le fonctionnement du moteur IA dans des conditions contrôlées. Il comprend trois documents académiques fictifs mais réalistes, représentatifs des situations que la plateforme devra traiter : un document source original (mémoire sur l'impact de l'IA sur l'enseignement supérieur en Afrique), un document plagiat (paraphrase systématique du source, avec reformulations lexicales et syntaxiques), et un document original sur un sujet totalement différent (étude de la biodiversité dans le bassin du Congo). Cette composition permet de tester à la fois la capacité de détection du plagiat (vrais positifs) et l'absence de faux positifs (le document original ne doit pas être signalé)."),
    ...threeLineTable(
      ["Document", "Type", "Statut", "Objectif"],
      [
        ["Impact de l'IA sur l'enseignement supérieur en Afrique", "Mémoire", "Source", "Document de référence"],
        ["L'IA dans les universités africaines : enjeux et perspectives", "Mémoire", "Plagiat", "Paraphrase du source"],
        ["Étude de la biodiversité dans le bassin du Congo", "TFC", "Original", "Sujet différent (test faux positifs)"],
      ],
      "Tableau IV.2 — Corpus pilote de test"
    ),
    H3("IV.10.2. Protocole d'évaluation"),
    P_body("Le protocole d'évaluation consiste à lancer l'analyse IA sur chacun des trois documents, en utilisant les deux autres comme corpus de comparaison. Le seuil de similarité est fixé à 0.15 (valeur par défaut de la plateforme). Pour chaque analyse, on mesure : le score global de similarité, le nombre de segments matchés, le nombre total de correspondances, la répartition par type de match, et le temps de traitement. Les résultats attendus sont : un score élevé pour le document plagiat, un score élevé (par réciprocité) pour le document source, et un score nul ou très faible pour le document original."),
    H3("IV.10.3. Résultats quantitatifs"),
    ...threeLineTable(
      ["Document analysé", "Score global", "Segments matchés", "Correspondances", "Temps (ms)"],
      [
        ["Impact de l'IA sur l'enseignement supérieur (source)", "77,8%", "2/2", "2", "1"],
        ["L'IA dans les universités africaines (plagiat)", "77,8%", "2/2", "2", "1"],
        ["Étude de la biodiversité Congo (original)", "0,0%", "0/2", "0", "1"],
      ],
      "Tableau IV.3 — Résultats de l'analyse IA sur le corpus pilote"
    ),
    P_body("Les résultats obtenus valident l'hypothèse formulée en introduction. Le document plagiat (« L'IA dans les universités africaines ») obtient un score de 77,8%, avec 2 segments sur 2 touchés et 2 correspondances détectées. Le document source obtient le même score par réciprocité (les similarités sont symétriques). Le document original sur la biodiversité obtient un score de 0%, ce qui confirme l'absence de faux positifs : la plateforme ne signale pas de similarités when les documents traitent de sujets réellement différents. Le temps de traitement est de 1 milliseconde pour chaque analyse, ce qui démontre l'efficacité de l'implémentation native TypeScript."),
    H3("IV.10.4. Analyse des correspondances détectées"),
    P_body("L'analyse détaillée des 4 correspondances détectées (2 par document plagiaire) révèle la pertinence de la classification. Les deux correspondances principales sont classées comme « traduction » (similarité sémantique 31,86%, lexicale 21,57%), ce qui correspond bien à une reformulation substantielle du texte source avec préservation du sens. Les deux correspondances secondaires sont classées « similarité faible » (sémantique 20,14%, lexicale 14,89%), correspondant à des passages partageant un vocabulaire technique commun mais avec une formulation différente. Cette classification fine permet à l'enseignant d'évaluer la gravité des similarités : une traduction est plus suspecte qu'une similarité faible."),
    ...threeLineTable(
      ...
      [
        ...
        ["1 (plagiat → source)", "Traduction", "31,86%", "21,57%", "Modérée"],
        ["2 (plagiat → source)", "Similarité faible", "20,14%", "14,89%", "Faible"],
      ],
      "Tableau IV.4 — Détail des correspondances détectées"
    ),
